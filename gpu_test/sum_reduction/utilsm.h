...
void bandwidth_print(unsigned long long duration, T catgr, unsigned int len) {
  ...
  if (bandwidth > (double)MB_SIZE) {
    std::cout << std::fixed << std::setprecision(3) << bandwidth / MB_SIZE << " MB/s" << std::endl;
  }
  else if (bandwidth > (double)KB_SIZE) {
    std::cout << std::fixed << std::setprecision(3) << bandwidth / KB_SIZE << " KB/s" << std::endl;
  }
  else {
    std::cout << std::fixed << std::setprecision(3) << bandwidth << " B/s" << std::endl;
  }
